...
def export_determinism_env(seed, directory, mode="gpu"):
  ...
  with open(os.path.join(directory, "determinism.json"), "w") as fp:
    json.dump(data, fp)

  return print("Your determinism environment is now saved to 'determinism.json'")

def set_determinism_env(determinism_env):
  # Change the input into dict / json
  for global_code in determinism_env["global_seed_code"]:
    exec("%s" % (global_code))

  for parallelism_ops in determinism_env["gpu_cpu_parallelism_ops"]:
    exec("%s" % (parallelism_ops))

def import_determinism_env(determinism_env_file):
